...
emulRes <- apsim_emulator(model = apsim_vector, X = X1, method = "singleGAM", exe = apsimExe, wd = apsimWD,
                        vars = vars, toRun = file, g = meanYield, overwrite = TRUE)
plot(emulRes)
@
...
cowpeaY <- apsim_vector(X = X1, exe = apsimExe, wd = apsimWD, vars = vars, toRun = file, g = rawYield, overwrite = TRUE)
