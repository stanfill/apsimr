...
<<edit,eval=FALSE>>=
edit_apsim(apsimFile, apsimWd, var, value, overwrite = FALSE)
@
...
emulRes <- apsim_emul_sa(model = apsim_vector, X = X1, method = "singleGAM", exe = apsimExe, wd = apsimWd,
                        vars = vars, to.run = apsimFile, g = meanYield, overwrite = TRUE)
plot(emulRes)
@
...
cowpeaY <- apsim_vector(X = X1, exe = apsimExe, wd = apsimWd, vars = vars, toRun = apsimFile, g = rawYield, overwrite = TRUE)
